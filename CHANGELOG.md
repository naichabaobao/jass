#### 1.9.19 (pre-release)
- 修复 textmacro 模板体语义误报：`$TYPE$` 等占位符在特定场景（如模板体所在解析流错位）下会以真实函数声明的形态漏进 AST，被当成普通代码检查后产生 `Invalid parameter type '$TYPE$'` 等必然误报。现语义分析器对**名字或类型名带 `$NAME$` 占位符的函数/native 声明整体跳过**（JASS 合法标识符不含 `$`，此类节点只可能来自模板体泄漏），类型有效性检查同样放行占位符。`analyzer.test.ts` 新增 2 个回归用例（正常模板不误报 + 泄漏场景不误报）。
