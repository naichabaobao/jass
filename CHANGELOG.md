...
1.4.0 对当前目录所有文件识别(单个目录文件上限为24个,后续可能会提高一点), 在only jass模式下是更准确的避开zinc块,而vjass在仅解析library struct method type。

解释一下为什么是24个,因为在某些情况下,用户会打开桌面某一个.j文件,而忽略掉桌面还有其他的文件夹，刚好这个文件夹里面有好多的小文件，这是会导致插件不停的往下递归，譬如node_modules这种文件夹，
所以得设置一个上限，超过了就不在解析后面的。
