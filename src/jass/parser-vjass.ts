...
export class TypeInferencer {
    ...
    private findLocalInFunction(func: NodeAst, name: string): string {
        const takes = (func as any).takes as Take[] | null | undefined;
        if (Array.isArray(takes)) {
            for (const take of takes) {
                if (take.name?.getText() === name) {
                    return take.type ? take.type.getText() : "unknown";
                }
            }
        }

        const searchLocalInNode = (node: NodeAst): string => {
            for (const child of node.children) {
                const maybeLocal = child as any;
                const isLocalLike =
                    child instanceof Local ||
                    (maybeLocal &&
                        maybeLocal.constructor &&
                        maybeLocal.constructor.name === "Local" &&
                        typeof maybeLocal.name?.getText === "function");

                if (isLocalLike) {
                    if (maybeLocal.name?.getText() === name) {
                        return maybeLocal.type ? maybeLocal.type.getText() : "unknown";
                    }
                }

                const nested = searchLocalInNode(child);
                if (nested !== "unknown") {
                    return nested;
                }
            }
        ...
        const localType = searchLocalInNode(func);
    ...
    private findStructMemberType(structType: string, memberName: string): string {
        ...
        const findInType = (typeName: string): string => {
            if (visited.includes(typeName)) {
                return "unknown";
            }
            visited.push(typeName);

            const structs = this.globalContext.get_structs_by_name(typeName);
            for (const struct of structs) {
                for (const child of struct.children) {
                    if (child instanceof Member || child instanceof zinc.Member) {
                        if (child.name?.getText() === memberName) {
                            return child.type ? child.type.getText() : "unknown";
                        }
                    }
                }

                const extendsList = (struct as any).extends as Token[] | null | undefined;
                if (Array.isArray(extendsList)) {
                    for (const extendToken of extendsList) {
                        const parentName = extendToken?.getText?.();
                        if (!parentName) {
                            continue;
                        }
                        const parentType = findInType(parentName);
