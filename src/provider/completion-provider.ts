...
import { AllKeywords, ZincKeywords } from '../vjass/keyword';
import { TextMacroRegistry } from '../vjass/text-macro-registry';
import { DefineRegistry } from '../vjass/define-registry';
import { CompletionCache, CustomCompletionItem } from './completion-cache';
...
export class CompletionProvider implements vscode.CompletionItemProvider {
    ...
    provideCompletionItems(
        ...
        try {
            ...
            if (memberAccessContext) {
                // 提取前缀（如果用户输入了部分成员名，如 obj.me）
                const prefix = this.extractMemberAccessPrefix(document, position);
                
                // 在成员访问上下文中，只提供该 struct/interface 的成员
                this.provideMemberAccessCompletion(memberAccessContext, items, itemSet, prefix);
            ...
            if (takesContext) {
                ...
                // 正常补全流程
                // 优化：从后往前查找单词边界，比正则表达式更快（提取到外部作用域，供后续使用）
                ...
                if (start < position.character - 1) {
                    prefix = textBeforeCursor.substring(start + 1, position.character);
                }
                ...
                // 2. 从补全项缓存中读取（优化：如果用户已输入，使用前缀过滤）
                try {
                    const completionCache = CompletionCache.getInstance();
                    
                    let allCachedItems: CustomCompletionItem<Statement>[];
                    if (prefix.length >= 2) {
                        // 如果用户已输入至少2个字符，使用前缀过滤（性能更好）
                        allCachedItems = completionCache.findByName(prefix);
                    } else {
                        // 否则获取所有补全项
                        allCachedItems = completionCache.getAll();
                    }
                ...
                // 3. 添加 TextMacro 补全项（优化：也支持前缀过滤）
                try {
                    const textMacroRegistry = TextMacroRegistry.getInstance();
                    const allMacros = textMacroRegistry.getAll();
                    
                    // 如果用户已输入前缀，过滤 TextMacro（与缓存补全项保持一致）
                    const filteredMacros = prefix.length >= 2
                        ? allMacros.filter(macro => 
                            macro.name.toLowerCase().startsWith(prefix.toLowerCase())
                          )
                        : allMacros;
                ...
                // 4. 添加 #define 补全项（优化：也支持前缀过滤）
                try {
                    const defineRegistry = DefineRegistry.getInstance();
                    const allDefines = defineRegistry.getAll();
                    
                    // 如果用户已输入前缀，过滤 #define（与缓存补全项保持一致）
                    const filteredDefines = prefix.length >= 2
                        ? allDefines.filter(define => 
                            define.name.toLowerCase().startsWith(prefix.toLowerCase())
                          )
                        : allDefines;
                    
                    filteredDefines.forEach(define => {
                        if (!itemSet.has(define.name)) {
                            const item = new vscode.CompletionItem(
                                define.name,
                            ...
                            // 优化多行 #define 的显示
                            let doc = `#define ${define.name}${valueStr}`;
                            
                            // 如果使用原始代码且是多行，进行格式化
                            if (define.code && define.code.includes('\n')) {
                                const lines = define.code.split('\n');
                                if (lines.length > 3) {
                                    doc = lines.slice(0, 3).join('\n') + '\n...';
                                } else {
                                    doc = define.code;
                                }
                            ...
                            documentation.appendMarkdown(`\n\n**_>:** \`${this.getRelativePath(define.filePath)}\``);
                            
                            item.documentation = documentation;
                            item.insertText = define.name;
                            item.sortText = `4_define_${define.name}`;
                            items.push(item);
                            itemSet.add(define.name);
                        }
                    });
                } catch (error) {
                    // 忽略错误
            ...
            // 5. 如果是当前文件，查找包含当前位置的函数/方法，提取局部变量和参数（需要实时计算）
    ...
    private getMemberAccessContext(
        ...
        // 需要匹配最后一个点号前的标识符，支持链式访问（如 obj.method().member）
        // 先找到最后一个点号的位置
        ...
        // 提取最后一个点号前的文本（可能是链式访问，如 obj.method()）
        ...
        // 1. obj. (简单访问)
        // 2. obj.method(). (方法调用)
        // 3. obj.method1().method2(). (链式方法调用)
        // 4. obj.field.method(). (字段访问后方法调用)
        return this.resolveChainAccess(document, position, beforeLastDot);
    }
    
    /**
     * 解析链式访问，返回最终的类型信息
     * 例如：obj.method() -> 返回 method 的返回类型
     *      obj.field.method() -> 返回 method 的返回类型
     */
    private resolveChainAccess(
        document: vscode.TextDocument,
        position: vscode.Position,
        chainExpression: string
    ): { structName: string; isStatic: boolean; isThis: boolean; isThistype: boolean } | null {
        // 检查是否是 this 或 thistype
        const trimmed = chainExpression.trim();
        const lowerTrimmed = trimmed.toLowerCase();
        
        if (lowerTrimmed === 'this') {
        ...
        // 解析链式访问：从右到左解析每个部分
        // 例如：obj.method().field.method() -> 需要解析：
        // 1. obj (根对象)
        // 2. method() (方法调用，返回类型)
        // 3. field (字段访问)
        // 4. method() (方法调用，返回类型)
        ...
        // 检查是否有方法调用（以 () 结尾）
        const methodCallMatch = chainExpression.match(/(\w+)\s*\(\s*\)\s*$/);
        if (methodCallMatch) {
            // 有方法调用，提取方法名和前面的表达式
            const methodName = methodCallMatch[1];
            const beforeMethod = chainExpression.substring(0, methodCallMatch.index).trim();
            
            // 递归解析前面的表达式，获取对象类型
            const objectContext = beforeMethod.length > 0
                ? this.resolveChainAccess(document, position, beforeMethod)
                : null;
            
            if (objectContext) {
                // 查找方法的返回类型
                const returnType = this.findMethodReturnType(
                    objectContext.structName,
                    methodName,
                    objectContext.isStatic
                );
                
                if (returnType) {
                    // 如果返回类型是 "thistype"，需要查找当前 struct
                    let typeName = returnType;
                    ...
                    // 检查返回类型是否是 struct/interface
                    if (this.isStructOrInterfaceType(typeName)) {
                        return {
                            structName: typeName,
                            isStatic: false, // 方法调用返回的是实例
                            isThis: false,
                            isThistype: false
                        };
                    }
        ...
        // 没有方法调用，检查是否有字段访问（包含点号）
        const dotIndex = chainExpression.lastIndexOf('.');
        if (dotIndex !== -1) {
            // 有字段访问，提取字段名和前面的表达式
            const fieldName = chainExpression.substring(dotIndex + 1).trim();
            const beforeField = chainExpression.substring(0, dotIndex).trim();
            
            // 递归解析前面的表达式
            const objectContext = beforeField.length > 0
                ? this.resolveChainAccess(document, position, beforeField)
                : null;
            
            if (objectContext) {
                // 查找字段的类型
                const fieldType = this.findFieldType(
                    objectContext.structName,
                    fieldName,
                    objectContext.isStatic
                );
                ...
                if (fieldType) {
                    // 如果字段类型是 "thistype"，需要查找当前 struct
                    ...
                    // 检查字段类型是否是 struct/interface
                    if (this.isStructOrInterfaceType(typeName)) {
                        return {
                            structName: typeName,
                            isStatic: false, // 字段访问返回的是实例
                            isThis: false,
                            isThistype: false
                        };
                    }
        ...
        // 没有点号和方法调用，这是一个简单的标识符
        // 匹配最后一个完整的标识符（单词字符序列）
        const identifierMatch = chainExpression.match(/(\w+)\s*$/);
        if (!identifierMatch) {
            return null;
        }
        ...
        // 先尝试查找变量类型（变量类型推断）
        const variableType = this.findVariableType(document, position, identifier);
        ...
        // 如果没有找到变量类型，可能是类型名（静态访问，如 StructName.）
        ...
        // 首字母大写，可能是类型名（静态访问）
        if (this.isStructOrInterfaceType(identifier)) {
            return {
    ...
    /**
     * 检查类型是否是 struct 或 interface
     */
    private isStructOrInterfaceType(typeName: string): boolean {
        ...
        for (const filePath of allCachedFiles) {
            ...
            // 检查是否是 interface
            if (this.findInterfaceInBlock(blockStatement, typeName) !== null) {
                return true;
            }
        }
    ...
    private findMethodReturnType(
        ...
        for (const filePath of allCachedFiles) {
            ...
            if (struct) {
                ...
                // 查找 module 中的方法
                const moduleMethod = this.findMethodInModules(struct, methodName, isStatic);
                if (moduleMethod) {
                    if (moduleMethod.returnType) {
                        return moduleMethod.returnType.toString();
                    }
                }
                
                // 查找内置方法
                const builtinReturnType = this.findBuiltinMethodReturnType(struct, methodName, isStatic);
            ...
            // 查找 interface
            const interface_ = this.findInterfaceInBlock(blockStatement, structName);
            if (interface_) {
            ...
            if (struct) {
                ...
                for (const member of struct.members) {
                    if (member instanceof VariableDeclaration) {
                        if (member.name && member.name.name === fieldName) {
                            // 检查 static 匹配
                            if (isStatic && !member.isStatic) {
                                continue;
                            }
                ...
                // 查找 module 中的字段
                const moduleField = this.findFieldInModules(struct, fieldName, isStatic);
                if (moduleField) {
                    if (moduleField.type) {
                        return moduleField.type.toString();
                    }
                }
            }
        }
    ...
    /**
     * 在 struct 实现的 module 中查找方法
     */
    private findMethodInModules(
        struct: StructDeclaration,
        methodName: string,
        isStatic: boolean
    ): MethodDeclaration | null {
    ...
    /**
     * 在 struct 实现的 module 中查找字段
     */
    private findFieldInModules(
        struct: StructDeclaration,
        fieldName: string,
        isStatic: boolean
        ...
        for (const member of struct.members) {
            if (member instanceof ImplementStatement) {
                ...
                for (const cachedFilePath of allCachedFiles) {
                    ...
                    if (module) {
                        for (const moduleMember of module.members) {
                            if (moduleMember instanceof VariableDeclaration) {
                                if (moduleMember.name && moduleMember.name.name === fieldName) {
                                    // 检查 static 匹配
                                    if (isStatic && !moduleMember.isStatic) {
                                        continue;
                                    }
     ...
     * 查找内置方法的返回类型
     */
    private findBuiltinMethodReturnType(
        struct: StructDeclaration,
        methodName: string,
        isStatic: boolean
    ): string | null {
        const structName = struct.name?.name || '';
        
        // allocate() - 静态私有方法
        if (isStatic && methodName === 'allocate') {
            return structName;
        }
        
        // create() - 静态方法
        if (isStatic && methodName === 'create') {
            // 检查是否有自定义的 create 方法
            ...
            if (createMethod && createMethod.returnType) {
                return createMethod.returnType.toString();
            }
        ...
        if (methodName === 'destroy') {
            ...
            if (customDestroy) {
                if (isStatic && customDestroy.isStatic && customDestroy.returnType) {
                    return customDestroy.returnType.toString();
                }
                if (!isStatic && !customDestroy.isStatic && customDestroy.returnType) {
                    return customDestroy.returnType.toString();
                }
        ...
        // deallocate() - 实例方法
        if (!isStatic && methodName === 'deallocate') {
            return 'nothing';
        }
        
        // onDestroy() - 实例方法
        if (!isStatic && methodName === 'onDestroy') {
            const onDestroyMethod = struct.members.find(
                m => m instanceof MethodDeclaration && !m.isStatic && m.name?.name === 'onDestroy'
            ) as MethodDeclaration | undefined;
            
            if (onDestroyMethod && onDestroyMethod.returnType) {
                return onDestroyMethod.returnType.toString();
            }
    ...
    /**
     * 提取成员访问的前缀（如 obj.me 中的 "me"）
     */
    private extractMemberAccessPrefix(
        document: vscode.TextDocument,
        position: vscode.Position
    ): string {
        const lineText = document.lineAt(position.line).text;
        ...
        // 提取点号后的文本（可能是部分成员名）
        const afterDot = textBeforeCursor.substring(lastDotIndex + 1).trim();
        
        // 匹配单词字符（标识符）
        const match = afterDot.match(/^(\w+)/);
    ...
    private provideMemberAccessCompletion(
        ...
        try {
            ...
            if (foundStruct && foundFilePath) {
                // 提供 struct 成员补全（包括内置方法和继承链）
                this.addStructMembers(foundStruct, items, itemSet, context.isStatic, foundFilePath, context.isThis, context.isThistype, prefix);
            } else if (foundInterface && foundFilePath) {
                // 提供 interface 成员补全
                this.addInterfaceMembers(foundInterface, items, itemSet, context.isStatic, foundFilePath, prefix);
            }
        } catch (error) {
    ...
    private addStructMembers(
        ...
        // 如果 struct 有继承，先添加父类的成员
        if (struct.extendsType) {
            const parentTypeName = struct.extendsType.name;
            const parentStruct = this.findStructInAllFiles(parentTypeName);
            if (parentStruct) {
                // 递归添加父类成员（使用父类的文件路径）
                const parentFilePath = this.findStructFilePath(parentTypeName) || filePath;
                this.addStructMembers(parentStruct, items, itemSet, isStatic, parentFilePath, false, false, prefix);
            } else {
                // 如果不是 struct，可能是 interface（struct 可以通过 extends 实现 interface）
                const parentInterface = this.findInterfaceInAllFiles(parentTypeName);
                if (parentInterface) {
                    // 添加接口成员
                    const parentFilePath = this.findInterfaceFilePath(parentTypeName) || filePath;
                    this.addInterfaceMembers(parentInterface, items, itemSet, isStatic, parentFilePath, prefix);
                }
            }
        }
        ...
        for (const member of struct.members) {
            if (member instanceof MethodDeclaration) {
                if (member.name && (!itemSet.has(member.name.name))) {
                    ...
                    item.documentation = documentation;
                    // 优化排序：方法优先，按类型和名称排序
                    const typePriority = member.isStatic ? '0' : '1'; // 实例方法优先
                    item.sortText = `${typePriority}_method_${member.name.name}`;
                    items.push(item);
                ...
                if (!itemSet.has(member.name.name)) {
                    ...
                    item.sortText = `${typePriority}_field_${member.name.name}`;
                    items.push(item);
                    itemSet.add(member.name.name);
                }
            }
        ...
        // 添加实现的 module 成员
        this.addModuleMembers(struct, items, itemSet, isStatic, filePath, prefix);
        
        // 添加内置方法
        this.addBuiltinMethods(struct, items, itemSet, isStatic, structName, filePath, prefix);
    }
    
    /**
     * 在所有文件中查找 struct
     */
    private findStructInAllFiles(structName: string): StructDeclaration | null {
        const allCachedFiles = this.dataEnterManager.getAllCachedFiles();
        for (const filePath of allCachedFiles) {
            const blockStatement = this.dataEnterManager.getBlockStatement(filePath);
            if (!blockStatement) {
            ...
            const found = this.findStructInBlock(blockStatement, structName);
            if (found) {
                return found;
            }
        }
     ...
     * 查找 struct 所在的文件路径
     */
    private findStructFilePath(structName: string): string | null {
        const allCachedFiles = this.dataEnterManager.getAllCachedFiles();
        for (const filePath of allCachedFiles) {
            const blockStatement = this.dataEnterManager.getBlockStatement(filePath);
            if (!blockStatement) {
     ...
     * 在所有文件中查找 interface
     */
    private findInterfaceInAllFiles(interfaceName: string): InterfaceDeclaration | null {
        const allCachedFiles = this.dataEnterManager.getAllCachedFiles();
        for (const filePath of allCachedFiles) {
            const blockStatement = this.dataEnterManager.getBlockStatement(filePath);
            if (!blockStatement) {
            ...
            const found = this.findInterfaceInBlock(blockStatement, interfaceName);
            if (found) {
                return found;
            }
        }
    ...
    /**
     * 查找 interface 所在的文件路径
     */
    private findInterfaceFilePath(interfaceName: string): string | null {
        const allCachedFiles = this.dataEnterManager.getAllCachedFiles();
        for (const filePath of allCachedFiles) {
            const blockStatement = this.dataEnterManager.getBlockStatement(filePath);
            if (!blockStatement) {
        ...
        for (const member of struct.members) {
            if (member instanceof ImplementStatement) {
                ...
                for (const cachedFilePath of allCachedFiles) {
                    ...
                    if (module) {
                        // 添加 module 的成员
                        this.addModuleMembersToItems(module, items, itemSet, isStatic, cachedFilePath, prefix);
                        break;
                    }
                }
            }
        ...
        for (const member of module.members) {
            if (member instanceof MethodDeclaration) {
                ...
                for (const cachedFilePath of allCachedFiles) {
                    ...
                    if (nestedModule) {
                        this.addModuleMembersToItems(nestedModule, items, itemSet, isStatic, cachedFilePath, prefix);
                        break;
                    }
                }
            }
    ...
    private addBuiltinMethods(
        ...
        if (isStatic && !itemSet.has('allocate')) {
            // 前缀过滤
            if (prefix && !'allocate'.toLowerCase().startsWith(prefix.toLowerCase())) {
                // 跳过
            } else {
        ...
        if (isStatic && !itemSet.has('create')) {
            // 前缀过滤
            if (prefix && !'create'.toLowerCase().startsWith(prefix.toLowerCase())) {
                // 跳过
            } else {
        ...
        if (!itemSet.has('destroy')) {
            // 前缀过滤
            if (prefix && !'destroy'.toLowerCase().startsWith(prefix.toLowerCase())) {
                // 跳过
            } else {
            // 查找自定义的 destroy 方法
            const customDestroy = struct.members.find(
        ...
        if (!isStatic && !itemSet.has('deallocate')) {
            // 前缀过滤
            if (prefix && !'deallocate'.toLowerCase().startsWith(prefix.toLowerCase())) {
                // 跳过
            } else {
        ...
        if (!isStatic && !itemSet.has('onDestroy')) {
            // 前缀过滤
            if (prefix && !'onDestroy'.toLowerCase().startsWith(prefix.toLowerCase())) {
                // 跳过
            } else {
    ...
    /**
     * 去重并排序补全项
     * 优化：itemSet 已经在循环中使用，所以这里不需要再次去重，只需要排序
     */
    private deduplicateAndSort(items: vscode.CompletionItem[], itemSet: Set<string>): vscode.CompletionItem[] {
        // itemSet 已经在循环中使用，所以这里不需要再次去重
        // 只需要确保所有项都有 sortText，然后排序即可
        const visibleItems: vscode.CompletionItem[] = [];

        // 为没有 sortText 的项设置默认值
        for (const item of items) {
            if (!this.applyApiVersionPreference(item)) {
                continue;
            }
        ...
        this.appendDeprecatedReplacementItems(visibleItems);
