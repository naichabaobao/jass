...
export class HoverProvider implements vscode.HoverProvider {
    ...
    private getMemberAccessInfo(
        ...
        // 先尝试按变量解析（local/参数/struct成员/全局变量）
        const variableType = this.resolveVariableTypeForMemberAccess(originalIdentifier, document, position);
        if (variableType) {
            return {
        ...
        // 再尝试按结构/接口名解析（静态访问）
        const isStaticCandidate = originalIdentifier[0] === originalIdentifier[0].toUpperCase();
        if (isStaticCandidate) {
            const hasTypeDefinition =
                this.findStructDefinition(originalIdentifier) !== null ||
                this.findInterfaceDefinition(originalIdentifier) !== null;
    ...
    private findMemberHover(
        ...
        for (const filePath of allCachedFiles) {
            ...
            const interfaceDecl = this.findInterfaceInBlock(blockStatement, info.structName);
            if (interfaceDecl) {
                for (const member of interfaceDecl.members) {
                    if (member instanceof MethodDeclaration) {
                        if (member.name && member.name.name === info.memberName) {
                            ...
                            if (info.isStatic && !member.isStatic) continue;
                            if (!info.isStatic && member.isStatic) continue;
                            return this.createMethodHoverContent(member, interfaceDecl.name?.name || 'unknown', filePath);
                        }
                    } else if (member instanceof VariableDeclaration) {
                        if (member.name && member.name.name === info.memberName) {
                            if (info.isStatic && !member.isStatic) continue;
                            if (!info.isStatic && member.isStatic) continue;
                            return this.createMemberVariableHoverContent(member, interfaceDecl.name?.name || 'unknown', filePath);
                        }
                    }
                }
            }
    ...
    private resolveVariableTypeForMemberAccess(
        variableName: string,
        document: vscode.TextDocument,
        position: vscode.Position
    ): string | null {
        ...
        const currentStruct = this.findCurrentStruct(document, position);
        const callable = this.findContainingFunctionOrMethod(currentBlock, position);

        if (callable) {
            for (const param of callable.parameters || []) {
                if (param.name?.name === variableName) {
                    const paramType = this.normalizeTypeName(param.type, currentStruct);
                    if (paramType) {
                        return paramType;
                    }
                }
            ...
            if (callable instanceof FunctionDeclaration || callable instanceof MethodDeclaration) {
                if (callable.body) {
                    const localType = this.findLocalVariableTypeInBlock(callable.body, variableName, position, currentStruct);
                    if (localType) {
                        return localType;
                    }
                }
        ...
        if (currentStruct) {
            for (const member of currentStruct.members) {
                if (member instanceof VariableDeclaration && member.name?.name === variableName) {
                    const memberType = this.normalizeTypeName(member.type, currentStruct);
                    if (memberType) {
                        return memberType;
                    }
                }
        ...
        for (const cachedFilePath of this.dataEnterManager.getAllCachedFiles()) {
            ...
            const globalType = this.findGlobalVariableTypeInBlock(blockStatement, variableName, currentStruct);
            if (globalType) {
                return globalType;
            }
        }
    ...
    private findLocalVariableTypeInBlock(
        block: BlockStatement,
        variableName: string,
        position: vscode.Position,
        currentStruct: StructDeclaration | null
    ): string | null {
        for (const stmt of block.body) {
            if (stmt instanceof VariableDeclaration && stmt.isLocal) {
                if (stmt.name?.name === variableName && this.isVariableBeforePosition(stmt, position)) {
                    return this.normalizeTypeName(stmt.type, currentStruct);
                }
                continue;
            }

            if (stmt instanceof BlockStatement && this.isPositionInRange(position, stmt.start, stmt.end)) {
                const nested = this.findLocalVariableTypeInBlock(stmt, variableName, position, currentStruct);
                if (nested) {
                    return nested;
                }
            }
    ...
    private findGlobalVariableTypeInBlock(
        block: BlockStatement,
        variableName: string,
        currentStruct: StructDeclaration | null
    ): string | null {
        for (const stmt of block.body) {
            if (stmt instanceof VariableDeclaration && !stmt.isLocal) {
                if (stmt.name?.name === variableName) {
                    const typeName = this.normalizeTypeName(stmt.type, currentStruct);
                    if (typeName) {
                        return typeName;
                    }
                }
            } else if (stmt instanceof LibraryDeclaration || stmt instanceof ScopeDeclaration) {
                const members = stmt instanceof LibraryDeclaration ? stmt.members : stmt.members;
                for (const member of members) {
                    if (member instanceof BlockStatement) {
                        const nested = this.findGlobalVariableTypeInBlock(member, variableName, currentStruct);
                        if (nested) {
                            return nested;
                        }
                    } else if (member instanceof VariableDeclaration && !member.isLocal && member.name?.name === variableName) {
                        const typeName = this.normalizeTypeName(member.type, currentStruct);
                        if (typeName) {
                            return typeName;
                        }
                    }
                ...
                const nested = this.findGlobalVariableTypeInBlock(stmt, variableName, currentStruct);
                if (nested) {
                    return nested;
                }
            }
    ...
    private normalizeTypeName(
        type: Identifier | ThistypeExpression | null | undefined,
        currentStruct: StructDeclaration | null
    ): string | null {
        if (!type) {
            return null;
        ...
        if (type instanceof Identifier) {
            typeName = type.name;
        } else if (type instanceof ThistypeExpression) {
            typeName = currentStruct?.name?.name || null;
        }
        ...
        if (this.findStructDefinition(typeName) || this.findInterfaceDefinition(typeName)) {
            return typeName;
        }
        return null;
    }
    ...
    private findInterfaceInBlock(block: BlockStatement, interfaceName: string): InterfaceDeclaration | null {
        for (const stmt of block.body) {
