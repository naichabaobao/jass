...
export class InlayHintsProvider implements vscode.InlayHintsProvider {
    ...
    private processMethodCall(
        ...
        const isThisLikeIdentifier = left instanceof Identifier && (left.name === 'this' || left.name === '');
        if (left instanceof ThistypeExpression || isThisLikeIdentifier) {
            ...
            if (methodRight instanceof Identifier && (methodName === 'evaluate' || methodName === 'execute')) {
                ...
                if (methodLeft instanceof Identifier) {
                    // 先尝试静态方法对象：StructName.staticMethod.evaluate()
                    // 若不是 struct 名，则按实例方法对象：instance.method.evaluate()
                    let originalMethodDef: { parameters: Array<{ name: string; type: string }>; returnType: string | null } | null = null;
                    if (this.findStructByNameGlobally(methodLeft.name)) {
                        originalMethodDef = this.findMethodDefinition(methodLeft.name, originalMethodName, filePath);
                    } else {
                        const varType = this.findVariableTypeInContext(methodLeft.name, filePath, position);
                        if (varType) {
                            originalMethodDef = this.findMethodDefinition(varType, originalMethodName, filePath);
                        }
                    }

                    if (originalMethodDef) {
                        if (originalMethodDef.parameters.length > 0 && callExpr.arguments.length > 0) {
                            for (let i = 0; i < Math.min(originalMethodDef.parameters.length, callExpr.arguments.length); i++) {
                                const arg = callExpr.arguments[i];
                                const param = originalMethodDef.parameters[i];
    ...
    private processAssignmentExpression(
        ...
        if (assignment.target instanceof Identifier) {
            ...
            // 为左值数组元素补充类型提示
            let indexTarget: Expression = assignment.target.left;
            while (indexTarget instanceof BinaryExpression && indexTarget.operator === OperatorType.Index) {
                indexTarget = indexTarget.left;
            }
            if (indexTarget instanceof Identifier) {
                varName = indexTarget.name;
                const varDecl = this.findVariableDeclarationInContext(varName, filePath, assignmentPosition);
                if (varDecl?.isArray && varDecl.type) {
                    targetType = varDecl.type.toString();
                }
            } else if (indexTarget instanceof BinaryExpression &&
                       indexTarget.operator === OperatorType.Dot) {
                const memberDecl = this.findMemberAccessVariableDeclaration(
                    indexTarget,
                    filePath,
                    assignmentPosition
                );
                if (memberDecl?.declaration?.isArray && memberDecl.declaration.type) {
                    targetType = memberDecl.declaration.type.toString();
                    varName = memberDecl.memberName;
                }
            }
            // 处理数组下标中的调用
    ...
    private findVariableTypeInContext(
        ...
        const variableDecl = this.findVariableDeclarationInContext(variableName, currentFilePath, position);
        if (variableDecl && variableDecl.type) {
            return variableDecl.type.toString();
        }

        // takes 参数不属于 VariableDeclaration，这里做参数链路兜底
    ...
    /**
     * 查找变量声明（按照 local -> takes -> globals 的顺序）
     */
    private findVariableDeclarationInContext(
        variableName: string,
        currentFilePath: string,
        position: vscode.Position
    ...
    private findFunctionDefinition(
        ...
        for (const filePath of allCachedFiles) {
            ...
            if (func) {
                if (!this.isStatementAllowedForApiVersion(func, filePath)) {
                    continue;
                }
    ...
    private findMethodDefinition(
        ...
        for (const cachedFilePath of allCachedFiles) {
            ...
            if (structDecl) {
                ...
                if (method) {
                    if (!this.isStatementAllowedForApiVersion(method, cachedFilePath)) {
                        continue;
                    }
    ...
    /**
     * 查找成员访问对应的变量声明（用于数组元素类型等需要 isArray 的场景）
     */
    private findMemberAccessVariableDeclaration(
        expr: BinaryExpression,
        filePath: string,
        position: vscode.Position
    ): { declaration: VariableDeclaration; memberName: string } | null {
        if (expr.operator !== OperatorType.Dot) {
            return null;
        }
        ...
        if (isThisAccess) {
            ...
            const memberDecl = this.findStructMemberVariableDeclaration(structDecl, memberName);
            return memberDecl ? { declaration: memberDecl, memberName } : null;
        }

        if (!(left instanceof Identifier)) {
            return null;
        }
        const objectType = this.findVariableTypeInContext(left.name, filePath, position);
        if (!objectType) {
            return null;
        }
        const memberDecl = this.findTypeMemberVariableDeclaration(objectType, memberName);
        return memberDecl ? { declaration: memberDecl, memberName } : null;
    }
     ...
     * 查找结构体成员声明（支持继承链）
     */
    private findStructMemberVariableDeclaration(
        structDecl: StructDeclaration,
        memberName: string
    ): VariableDeclaration | null {
        for (const member of structDecl.members) {
            if (member instanceof VariableDeclaration && member.name && member.name.name === memberName) {
                return member;
            }
        }

        if (structDecl.extendsType) {
            const parentStruct = this.findStructByNameGlobally(structDecl.extendsType.name);
            if (parentStruct) {
                return this.findStructMemberVariableDeclaration(parentStruct, memberName);
            }
        }
    ...
    /**
     * 查找类型成员变量声明（struct/interface）
     */
    private findTypeMemberVariableDeclaration(
        typeName: string,
        memberName: string
    ): VariableDeclaration | null {
        const allCachedFiles = this.dataEnterManager.getAllCachedFiles();
