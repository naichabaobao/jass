...
export class ZincCompletionProvider implements vscode.CompletionItemProvider {
    ...
    private isStatementAllowedForApiVersion(stmt: ZincStatement, filePath: string): boolean {
    ...
    private extractSinceVersionFromStatement(stmt: ZincStatement, filePath: string): string | null {
        if (!stmt.start) {
            return null;
        }
    ...
    private addLibraryMemberCompletions(
        ...
        for (const { program: prog, filePath: progFilePath } of programs) {
            for (const stmt of prog.declarations) {
                if (!this.isStatementAllowedForApiVersion(stmt, progFilePath)) {
                    continue;
                }
