...
endstruct`,
        (errors) => {
            // 模块方法检查可能在调用时进行
            // 如果 this 类型无法解析，当前实现会给出对应警告
            return errors.errors.some(e =>
                e.message.includes("requiredMethod") || 
                e.message.includes("not found") ||
                e.message.includes("方法")
            ) || errors.warnings.some(w =>
                w.message.includes("Cannot resolve object type for 'this'") ||
                w.message.includes("unable to verify if method 'requiredMethod' exists")
            );
        }
    );
...
call BJDebugMsg(ts[0])
        ...
        (errors) => {
            // 当前解析器对 []= 仍有语法限制，至少应给出稳定的解析报错而非崩溃
            return errors.errors.length === 0 || errors.errors.some(e =>
                e.message.includes("Expected 'takes'") ||
                e.message.includes("Expected parameter type") ||
                e.message.includes("Expected 'returns'") ||
                e.message.includes("already declared")
            );
        }
    );
...
function TransferTimer takes nothing returns nothing
    ...
    console.log("=============================\n");
    if (totalFailed > 0) {
        throw new Error(`Analyzer tests failed: ${totalFailed}`);
    }
}
