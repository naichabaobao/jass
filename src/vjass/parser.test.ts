...
endlibrary
    ...
    try {
        ...
        const topLevelZincBlocks = resultZinc.body.filter((s: Statement) => s instanceof ZincBlockStatement) as ZincBlockStatement[];
        if (topLevelZincBlocks.length > 0) {
            const zincBlock = topLevelZincBlocks[0];
            console.log(`  顶层 Zinc 内容长度: ${zincBlock.content.length}`);
            console.log(`  顶层 Zinc 语句数量: ${zincBlock.zincStatements.length}`);
        } else {
            console.log("  顶层未直接出现 ZincBlockStatement（已在 library 成员中验证）。");
        }
    } catch (error: any) {
...
endstruct`,
    ...
    // 测试 10: loop 语句
    console.log("\n测试 10: loop 语句");
    const { LoopStatement, ExitWhenStatement } = require('./ast');
...
function test takes nothing returns nothing
    ...
    if (runtimeFailedCount > 0) {
        throw new Error(`Parser tests failed: ${runtimeFailedCount}`);
    }
}
