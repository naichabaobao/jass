...
export class Parser {
    ...
    public parse(): BlockStatement {
        ...
        while (!this.isAtEnd()) {
            ...
            try {
                ...
                if (stmt) {
                    this.appendStatement(statements, stmt);
                } else {
                    // 如果无法解析，尝试同步到下一个同步点
     ...
     * 从 token 中提取预处理器指令文本。
     * 支持两种来源：
     * - TEXTMACRO_DIRECTIVE（值为 " inject main" 这类内容）
     * - 注释 token（值形如 "//! inject main"）
     */
    private getDirectiveText(token?: IToken | null): string | null {
        if (!token) return null;

        if (token.type === TokenType.TextMacroDirective) {
    ...
    /**
     * 追加语句时处理 runtextmacro 展开块扁平化。
     */
    private appendStatement(target: Statement[], stmt: Statement): void {
        if (stmt instanceof BlockStatement && stmt.fromTextMacroExpansion) {
            target.push(...stmt.body);
            return;
        }
        target.push(stmt);
    }
    ...
    private parseStatement(): Statement | null {
        ...
        if (this.isTextMacroDirective()) {
            ...
            const directiveValue = this.getDirectiveText(directiveToken) || "";

            // 检查是否是 //! inject 指令
            const injectMatch = directiveValue.match(/^inject\s+(main|config)\s*$/i);
            if (injectMatch) {
                return this.parseInject(injectMatch[1] as "main" | "config");
            }

            // 检查是否是 //! endinject 指令（应该由 parseInject 处理，这里不应该单独出现）
            if (/^endinject\s*$/i.test(directiveValue)) {
                this.error(
            ...
            // 检查是否是 //! loaddata 指令
            const loadDataMatch = directiveValue.match(/^loaddata\s+"([^"]+)"\s*$/i);
            if (loadDataMatch) {
                const filePath = loadDataMatch[1];
                const startPos = directiveToken.start;
                const endPos = directiveToken.end;
                this.lexer.next();
    ...
    private parseStruct(): StructDeclaration | null {
        ...
        while (!this.isAtEnd()) {
            ...
            if (token) {
                ...
                const isConstantKeyword = this.check(TokenType.keywordConstant);
                const peekVal = this.lexer.peek()?.value?.toLowerCase();
                const isStaticMethodOrStub = isStaticKeyword && (peekVal === "method" || peekVal === "stub");
                if ((isTypeKeyword || isIdentifier || isStaticKeyword || isConstantKeyword) && !isStaticMethodOrStub) {
            ...
            if (member) {
                this.appendStatement(members, member);
            } else {
    ...
    private parseInject(injectType: "main" | "config"): Statement | null {
        ...
        while (!this.isAtEnd()) {
            ...
            // 检查是否是 //! endinject
            const directiveText = this.getDirectiveText(token);
            if (directiveText && /^endinject\s*$/i.test(directiveText)) {
                endPos = token.end;
    ...
    private parseZincBlock(): Statement | null {
        ...
        while (!this.isAtEnd()) {
            ...
            // 检查是否是 //! endzinc
            const directiveText = this.getDirectiveText(token);
            if (directiveText && /^endzinc\b/i.test(directiveText)) {
                endPos = token.end;
    ...
    private parseVariableDeclaration(
        ...
        if (this.checkValue("array")) {
            ...
            const isKeyType = typeToken.type === TokenType.TypeKey || (
                typeToken.type === TokenType.Identifier &&
                typeToken.value.toLowerCase() === "key"
            );
            if (isKeyType) {
                this.error(
